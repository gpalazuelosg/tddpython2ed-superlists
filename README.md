...
La razón de documentar es sencilla:
  - Repetir exactamente los pasos para habilitar, en 1 hora, una nueva Virtual Machine (VM) con Ubuntu Desktop 20.04 con Python 3.8.2 para desarrollar.
  - NVM y NodeJS
  - VSCode 
...
Además:
  - Go
  - Docker
  - (linuxbrew) PHP, Laravel, Composer
  - Postgres
...
> Para continuar, asumo que ya tienes VirtualBox instalado. Te has descargado, instalado y actualizado `Linux Ubuntu Desktop 20.04`. De lo contrario, no continues leyendo.
...
#### Instalar Python 3.8 (default instalado en Ubuntu 20.04)
...
```
$ cd ~/Downloads/
$ sudo curl -O https://dl.google.com/go/go1.14.4.linux-amd64.tar.gz
...
```
En una segunda terminal ejecutar lo siguiente:
```bash
$ python functional_tests.py 
```

Así, un ventana de firefox es abierta por Selenium mediante Geckodriver. La prueba falla, pero el resultado de habilitar el ambiente es el esperado. Ahora, puedes terminar tu virtual environment.
```bash
$ deactivate
```
...
##### Ahora si, instalar Docker CE engine
```bash
$ sudo apt-get update
...
##### (_mi preferido_) Instalar postgresql desde repositorio Ubuntu
Tiene versión más nueva. Básicamente hay que actualizar la GPG key en el catálogo, agregar repositorio apt a local y bajar la versión más reciente usando apt normal.

Este es el que ejecuto.
...
https://www.starwindsoftware.com/blog/how-to-install-django-on-ubuntu-server-18-04-lts-part-1-development-environment-installation
...
https://medium.com/@nitinraturi/how-to-set-up-django-with-postgres-nginx-and-gunicorn-on-ubuntu-14-04-77146b5565a0
...
Si el comando: ```$ pg_ctlcluster 12 main start``` falla, puedo ejecutar lo siguiente:
```bash
...
Me ayudaron:
  ...
  - https://linoxide.com/linux-how-to/setup-django-postgres-nginx-gunicorn-ubuntu/
  - https://linoxide.com/linux-how-to/setup-django-postgres-nginx-gunicorn-ubuntu/
  

  - https://www.starwindsoftware.com/blog/how-to-install-django-on-ubuntu-server-18-04-lts-part-1-development-environment-installation

  - https://www.starwindsoftware.com/blog/how-to-install-django-on-ubuntu-server-18-04-lts-part-2-configuring-django-to-use-mysql-database

  - https://www.starwindsoftware.com/blog/how-to-install-django-on-ubuntu-server-18-04-lts-part-3-configuring-django-to-use-postgresql-database


AHORA, despues puedo continuar:
```bash
...
```bash
Postgres=# CREATE USER newprojectuser WITH ENCRYPTED  PASSWORD 'password';
```
...
O, otro modo de crear base de datos y usuarios con credenciales es:
```bash
$ su postgres
# con esto estamos como root
$ createdb nombre_basedatos
$ createuser nombre_usuario
# ahora entrar a la base de datos
$ psql nombre_basedatos
# ahora estoy en postgres, dentro de nombre_basedatos
nombre_basedatos=# alter user nombre_usuario with password 'me-secured-password'';
ALTER ROLE
nombre_basedatos=# /q
# ahora me sali de postgres y estoy como root nuevamente
$ exit # para salir del usuario postgres
# ahora estoy en shell en mi virtualmente (si hubiese)

```

Al conectarme a Django, tendre que usar:
```bash
(virtual-environment) pip3 install psycopg2-binary
```
Y utilizar la siguiente conexion:
